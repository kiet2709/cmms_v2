...
async function fetchEquipments(page = 1, limit = 10) {
  try {
    loading.value = true;
    const res = await axiosClient.get(`daily-plans?page=${page}&limit=${limit}`);
    ...
    const apiData = res.data.data.map(item => {
      ...
      const { uuid, id, equipment_id, equipment_category_id, ...rest } = flat;
...
function customRow(record) {
  return {
    onClick: () => {
      ...
      router.push(`/dashboard/daily-inspection-plans/${record.uuid}`);
    }
  };
}
