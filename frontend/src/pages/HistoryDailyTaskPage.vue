...
async function fetchEquipments(page = 1, limit = 10) {
  try {
    ...
    const res = await axiosClient.get(`/daily-tasks/history?fromDate=${fromDate}&toDate=${toDate}&page=${page}&limit=${limit}`);
...
function customRow(record) {
  return {
    onClick: () => {
      dailyTaskStore.setUuid(record.uuid)
      router.push(`/dashboard/daily-inspection-plans/history/${record.uuid}`);
    }
  };
}
